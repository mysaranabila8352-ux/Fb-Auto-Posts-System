...
// Converts a stored UTC "HH:MM" (used for daily recurrence) back into the
// viewer's local time, for display purposes only.
  ...
  try {
    ...
    const rows = posts.slice(0, 8).map(p => `
      <tr>
        ...
        <td class="timestamp">${p.recurrence === 'daily' ? `Daily @ ${utcHHMMToLocal(p.daily_time)}` : fmt(p.scheduled_time)}</td>
      </tr>
    `).join('');
...
form.addEventListener('submit', async (e) => {
  ...
  const scheduledLocalValue = document.getElementById('f-scheduled').value;
  const dailyLocalTime = document.getElementById('f-daily-time').value; // "HH:MM" in browser's local time

  // Convert a local "HH:MM" into the equivalent UTC "HH:MM" so the server
  // (which compares against its own UTC clock) fires at the right local moment.
  function localTimeToUtcHHMM(hhmm) {
    if (!hhmm) return '';
    const [h, m] = hhmm.split(':').map(Number);
    const now = new Date();
  ...
  const payload = {
    ...
    recurrence,
    // datetime-local inputs have no timezone info — new Date() here interprets
    // the value in the BROWSER's local timezone, and toISOString() converts
    // it to the correct UTC instant before sending to the server.
    scheduled_time: scheduledLocalValue ? new Date(scheduledLocalValue).toISOString() : '',
    daily_time: localTimeToUtcHHMM(dailyLocalTime)
  };
